...
% Note that the process generating the input for \sort is most likely halted once the memory limit of \sort is reached. This is due to the buffer offered by the pipeline being full and thus the write operation being blocked. \sort only empties the buffer again after a temporary file is written. 
Pipelining the output of \sort eliminates the need for both compression and writing to disk in the output stage of \sort. \\

Testing pipelining for \sort with chaining \sort with SAMtools \texttt{view},  
