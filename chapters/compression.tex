...
\subsection{Analysis}
Running on 16 cores, with a total of 32\,GiB of memory, \sort takes 71 minutes and 57 seconds to sort a 215\,GB BAM file. However, it uses only 2 minutes and 35 seconds, which are 3.6\% of the total time, for sorting (merging not included). The profiler VTune \cite{noauthor_fix_nodate} reveals that much of the remaining time is dedicated to compression. \\ 
...
\begin{figure}
    ...
    \label{fig:bgzfspeed}
\end{figure}
To work around 7BGZF's limitation regarding separate compression levels for output and temporary files, I tested it on a file small enough to avoid producing temporary files in \sort. The results can still be used to see which libraries have potential for replacing zlib in HTSlib. \\
The best performance could be reached by igzip. Both tested compression levels performed better than all compression levels of all the other compression libraries. However, the compression rate of both tested compression levels turned out worse than libdeflate on compression level 1 (30\% respective 23\% compression ratio for igzip on compression level 1 respective 3 against 22\% compression ratio for libdeflate on compression level one, see Figure \ref{fig:bgzfComps}). \\
While slz takes approximately the same time as igzip on level 3, its compression rate of 30\% is more comparable to igzip on compression level 1. \\
